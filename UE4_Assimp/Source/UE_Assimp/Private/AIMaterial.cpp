...
FString UAIMaterial::GetTexturePath(EAiTextureType TextureType) const
{
	if (Material)
	{
		aiString TexturePath;
		if (Material->GetTexture((aiTextureType)TextureType, 0, &TexturePath) == AI_SUCCESS)
		{
			return FString(TexturePath.C_Str());
		}
	}
	return FString();
}

bool UAIMaterial::HasTexture(EAiTextureType TextureType) const
{
	if (Material)
	{
		aiString TexturePath;
		return Material->GetTexture((aiTextureType)TextureType, 0, &TexturePath) == AI_SUCCESS;
	}
	return false;
}