...
DECLARE_DYNAMIC_DELEGATE_OneParam(
	FOnImportSceneComplete,
	const TArray<UAIScene*>&, Scenes
);

UCLASS()
class UE_ASSIMP_API UAssimpImportContext : public UObject
{
	GENERATED_BODY()
...
public:
	...
	FThreadSafeCounter RunningTasks;

	bool bCancelled = false;

	TArray<FString> PendingFiles;
	UPROPERTY()
	TArray<TObjectPtr<UAIScene>> ResultScenes;
	UPROPERTY()
...
FileTypes					The type filters to show in the dialog. This string should be a "|" delimited list of (Description|Extensionlist) pairs. Extensionlists are ";" delimited.
 ...
 Success  true if files were successfully selected	*/
	...
	//Experimental
	UFUNCTION(BlueprintCallable, Category = "Assimp", meta = (WorldContext = "WorldContextObject"))
	static UAssimpImportContext* ImportScenesAsync(
	...
	static void CancelImport(UAssimpImportContext* Context);

	static FTransform aiMatToTransform(aiMatrix4x4 NodeTransform);

	static bool ImportTextureAsyncC(
		UObject* WorldContextObject,
	...
	UFUNCTION(BlueprintCallable, Category = "Texture Import", meta = (WorldContext = "WorldContextObject"))
	static bool ImportTextureAsync(
		UObject* WorldContextObject,
		EAiTextureType TextureType = EAiTextureType::AiTextureType_NONE,
		FName DynamicMaterialParamName = "",
		UAIScene* AssimpScene = nullptr,
		UAIMaterial* AssimpMaterial = nullptr,
		UMaterialInstanceDynamic* DynamicMaterialUnreal = nullptr
	);

	UFUNCTION(BlueprintCallable, Category = "Texture Import", meta = (WorldContext = "WorldContextObject"))
	static void ProcessNodes(UObject* WorldContextObject, UAINode* RootNode, const TArray<UMaterialInstanceDynamic*>& Materials);

	UFUNCTION(BlueprintCallable, Category = "Texture Import", meta = (WorldContext = "WorldContextObject"))
	static void CreateScences(UObject* WorldContextObject, TArray<UAIScene*> AIScenes, class UMaterialInterface* Parent);
	...
	*/
	...
	UFUNCTION(BlueprintCallable, Category = "Assimp")
	static FString GetTexturePath(const UAIScene* Scene, const UAIMaterial* Material, EAiTextureType TextureType);


	UFUNCTION(BlueprintCallable, Category = "Assimp")
	static bool HasTexture(const UAIScene* Scene, const UAIMaterial* Material, EAiTextureType TextureType);
